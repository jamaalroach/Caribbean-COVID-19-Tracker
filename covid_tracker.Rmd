...
output: 
  flexdashboard::flex_dashboard:
    orientation: row
    social: [ "twitter", "facebook", "instagram", "whatsapp" ]
    theme: spacelab
...
carib_data$Country.Region <-
  as.character(carib_data$Country.Region) #convert factor to character
carib_data$country <-
  if_else(
    carib_data$Province.State == "",
    carib_data$Country.Region,
...
carib_data$logcases<-log(carib_data$cases)
...
######create dataframe with toal cases per day per country
...
t_cases<-carib_data%>%filter(date==today)%>%summarise(sum(cases))
valueBox(t_cases, color = "warning")
...
```{r}
t_deaths<-carib_data%>%filter(date==today)%>%summarise(sum(`death_data$deaths`))
valueBox(t_deaths, color = "danger")
...
valueBox(total_recov, color = "success")
...
p("A daily tracker of the COVID-19 pandemic cases and other data from the countries in the Caribbean.")

p(tags$b(" View and compare trends by selecting either the Confirmed Cases or Deaths tab and then Country(ies)."))

p(tags$b("Error message indicates that the country has no cases / deaths.", style="color:red"))

###selectInput("outcome", label = "Metric",
            ###choices = c("Confirmed Cases"="cases","Deaths"="'death_data$deaths'"))
...
pickerInput(
  ...
  choices = c(
    ...
    "Venezuela",
    "Bermuda"
  ), options = list(`actions-box` = TRUE), selected = "Antigua and Barbuda", multiple = T
)


p("Source data from John Hopkins University CSSE 2019 Novel Coronavirus COVID-19 (2019-nCoV) Data Repository.  ")

p("The tracker will be updated daily and is provided for informational purposes only.")



p("Please send questions, queries or comments to sunrumdata@gmail.com.")
...
renderPlotly({
  p3<-ggplot() +
      geom_line(data = plot_data()%>%filter(cases>0), aes(
      x = date, y = cases, colour = country
    )) +
      geom_point(data = plot_data()%>%filter(cases>0),
                 aes(
                   x = date,
                   y = cases,
                   colour = country
      ...
      labs(
        title = "",
        caption = "Source: JHU CCSE Data Repository",
        subtitle = "As at March 25th, 2020",
        y = "Confirmed Cases",
        x = "Date"
      )
  ggplotly(p3)
...
renderPlotly({
  p4<-ggplot() +
      geom_line(data = plot_data()%>%filter(`death_data$deaths`>0), aes(
      x = date, y = `death_data$deaths`, colour = country
    )) +
      geom_point(data = plot_data()%>%filter(`death_data$deaths`>0),
                 aes(
                   x = date,
                   y = `death_data$deaths`,
                   colour = country
...
DT::datatable(carib_data_today %>%select(country, cases, `death_data$deaths`), colnames = c("Country", "Confirmed Cases", "Deaths"), class = "cell-border stripe", style = "bootstrap", rownames = FALSE)
```
